...
@frappe.whitelist()
def set_net_total(doc, methhod):
    doc.net_amount = doc.total_allocated_amount


@frappe.whitelist()
def calc_tax(tax_cat,amount):
    tax = frappe.get_doc("Tax Withholding Category", tax_cat)
    tax_rate = 0
    for rates in tax.rates:
        tax_rate += rates.tax_withholding_rate
    tax_amount = (tax_rate * float(amount))/100
    total_amount = tax_amount + float(amount)
    return tax_amount,total_amount


@frappe.whitelist()
def set_bill_details(doc, method=None):
...
def validate_chq(doc, methhod=None):
    if frappe.db.exists("Payment Entry", {"docstatus":1, "reference_no":doc.reference_no}):
