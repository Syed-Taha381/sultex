	...
	def get_raw_materials(self):
		...
		if self.filters.based_on == "Work Order":
			...
			if self.filters.item_group:
				filters.update({
					"item_group": self.filters.item_group
				})
			raw_materials = (
				frappe.get_all(
					"Work Order Item",
			...
			ic_cond = ""
			if self.filters.item_group:
				ic_cond = " and bom_item.item_group = '{group}'".format(group=self.filters.item_group)
			raw_materials = frappe.db.sql(
				...
				WHERE
					bom_item.parent in ({2}) and bom_item.parent = bom.name and bom.docstatus = 1 {3}
			""".format(
					qty_field, bom_doctype, ",".join(["%s"] * len(bom_nos)), ic_cond
				),
				tuple(bom_nos),
				as_dict=1,
			)
