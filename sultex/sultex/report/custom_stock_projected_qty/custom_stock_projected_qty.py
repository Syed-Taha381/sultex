# Copyright (c) 2013, Havenir Solutions pvt Limited and contributors
...
def execute(filters=None):
	...
	for bin in bin_list:
		...
		# getting all descendants of selected item group
		if filters.get("item_group"):
			item_group = filters.get("item_group")
			group_list = []
			get_item_groups(item_group, group_list)
  
		if filters.brand and filters.brand != item.brand:
			continue
		#Here
		elif filters.item_group and item.item_group not in group_list:
			continue
...
def get_item_groups(item_group, group_list):
    # using recursion
	doc = frappe.get_doc("Item Group", item_group)
	if not doc.is_group:
		group_list.append(doc.name)
	else:
		group_list.append(doc.name)
		item_groups = frappe.get_list("Item Group", {"parent_item_group": doc.name})
		for row in item_groups:
			get_item_groups(row.name, group_list)
